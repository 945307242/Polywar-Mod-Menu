...
std::string checkmenustatus(char* link,int timeout = 5000) {
    std::string status; // note to self: this initializes with value ""
    try {
        //http::Request request{link};
        //const auto response = request.send("GET","",{},std::chrono::milliseconds{timeout});
        //status = std::string{response.body.begin(), response.body.end()};
        //httplib::Client cli(link);
        //auto res = cli.Get("");
        //status = res->body;
    }
    catch (const std::exception &e) {
        status = "[GetStatusFailed]";
    }
    return(status);
}
    ...
    //Set sub title
    setText(env, title, OBFUSCATE("<b>Modded by HorridModz</b>"));

    //Set sub title
    setText(env, subtitle, OBFUSCATE("<b><marquee><p style=\"font-size:30\">"
                                     "<p style=\"color:green;\">https://github.com/HorridModz | Discord: Discord: User123456789#6424</p>"
                                     "</marquee></b>"));
    ...
    //Toast Example
    Toast(env,ctx,"Modded by HorridModz\nDiscord: User123456789#6424",ToastLength::LENGTH_LONG);
    menuauthorized = true; //disables system
    menuenabled = true; //disables system
    if (!menuauthorized) {
        std::string status = checkmenustatus(
                "https://github.com/HorridModz/Polywar-Mod-Menu/edit/main/menustatus.txt"); // Check to see if the menu is enabled.
        Toast(env,ctx,status.c_str(),ToastLength::LENGTH_LONG);
        sleep(2.5);
        if (status == "[GetStatusFailed]") {
            std::string message = "The menu could not be authorized. Make sure you have a stable internet connection.";
            Toast(env, ctx, message.c_str(), ToastLength::LENGTH_LONG);
            sleep(2.5);
            //forcecrash();
            menuenabled = false;
        } else if (status != "OK") {
            std::string basemessage = "Sorry, the menu is currently disabled: ";
            std::string message = basemessage + status;
            Toast(env, ctx, message.c_str(), ToastLength::LENGTH_LONG);
            sleep(2.5);
            //forcecrash();
            menuenabled = false;
